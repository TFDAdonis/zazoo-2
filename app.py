...
def render_guide():
    if st.session_state.show_guide and st.session_state.current_step in GUIDES:
        ...
        if st.session_state.current_guide_step >= len(guides) - 1 and st.session_state.current_step >= 5:
...
<div style="margin-bottom: 20px;">
    ...
    <div style="display: flex; align-items: center; gap: 10px; margin-top: 10px;">
        ...
        <button onclick="parent.postMessage({'type': 'streamlit:setComponentValue', 'value': 'toggle_guide'}, '*')" style="margin-left: auto; background: transparent; color: #00ff88; border: 1px solid #00ff88; padding: 4px 12px; border-radius: 20px; font-size: 12px; cursor: pointer;">
            """ + ("👁️ Hide" if st.session_state.show_guide else "👋 Show") + """ Guide
        </button>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col2:
    ...
    if st.session_state.current_step <= 3:
        ...
        <body>
          ...
          {f'''<div class="guide-pointer"></div>''' if st.session_state.show_guide and st.session_state.current_step == 3 and st.session_state.current_guide_step == 0 else ''}
