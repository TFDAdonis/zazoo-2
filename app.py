...
def create_comprehensive_climate_charts(self, climate_df, location_name):
    """Create comprehensive climate analysis charts with fixed array length issues"""
    ...
    if 'volumetric_soil_water_layer_1' in climate_df.columns:
        ...
        # Create month labels for all 12 months
        ...
        for i, (layer, label, color) in enumerate(zip(layer_names, layer_labels, colors)):
            if layer in climate_df.columns:
                ...
                monthly_data = climate_df.groupby('month')[layer].mean()
                
                # Create array for all months, fill missing months with NaN
                monthly_values = []
                ...
                valid_indices = [i for i, v in enumerate(monthly_values) if not np.isnan(v)]
                valid_values = [v for v in monthly_values if not np.isnan(v)]
                
                if valid_indices:
                    ax1.plot([x_positions[idx] for idx in valid_indices], valid_values, 
    ...
    # 2. Monthly Water Balance Chart - FIXED VERSION
    ...
    if 'total_precipitation' in climate_df.columns and 'potential_evaporation' in climate_df.columns:
        ...
        # Calculate monthly precipitation and evaporation
        ...
        for month in all_months:
            if month in precip_monthly.index:
                precip_values.append(precip_monthly[month])
            else:
                precip_values.append(0)
            
            if month in evap_monthly.index:
                evap_values.append(evap_monthly[month])
            else:
                evap_values.append(0)
        
        width = 0.35
        ax2.bar([i - width/2 for i in x_positions], precip_values, width, 
                label='Precipitation', color='#36A2EB', alpha=0.8)
        ax2.bar([i + width/2 for i in x_positions], evap_values, width, 
    ...
    # 3. Seasonal Water Balance Chart - FIXED VERSION
    ...
    if 'total_precipitation' in climate_df.columns and 'potential_evaporation' in climate_df.columns:
        ...
        # Plot lines
        ax3.plot(all_months, precip_values, 'b-', linewidth=2, label='Precipitation', marker='o')
        ax3.plot(all_months, evap_values, 'r-', linewidth=2, label='Evaporation', marker='s')
        
        # Fill between for water surplus/deficit
        ax3.fill_between(all_months, precip_values, evap_values, 
    ...
    # 4. Summary Statistics Panel (unchanged)
    ...
    def run_enhanced_climate_soil_analysis(self, geometry, location_name):
        ...
        try:
            ...
            # Get soil analysis
            soil_results = self.run_comprehensive_soil_analysis(
                location_name.split(',')[-1].strip(),
                location_name.split(',')[-2].strip() if ',' in location_name else 'Select Region',
                location_name.split(',')[0].strip() if location_name.count(',') >= 2 else 'Select Municipality'
            )
