...
class ComprehensiveAgriculturalAnalyzer:
    ...
    def create_matplotlib_visualization(self, df, location_name):
        """Create matplotlib visualizations as alternative to seaborn"""
        try:
            # Create figure with subplots
            fig, axes = plt.subplots(2, 2, figsize=(15, 10))
            
            # Plot 1: Suitability Scores
            colors = ['green' if s > 0.7 else 'orange' if s > 0.5 else 'red' for s in df['Suitability Score']]
            axes[0, 0].barh(df['Crop'], df['Suitability Score'], color=colors, alpha=0.7)
            axes[0, 0].set_xlabel('Suitability Score')
            axes[0, 0].set_title(f'Crop Suitability Scores - {location_name}')
            axes[0, 0].set_xlim(0, 1)
            axes[0, 0].grid(True, alpha=0.3)
            
            # Add value labels
            for i, (bar, score) in enumerate(zip(axes[0, 0].patches, df['Suitability Score'])):
                axes[0, 0].text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                              f'{score:.2f}', va='center', fontsize=8)
            
            # Plot 2: Disease Risk
            colors = ['red' if r > 0.6 else 'orange' if r > 0.3 else 'green' for r in df['Disease Risk']]
            axes[0, 1].barh(df['Crop'], df['Disease Risk'], color=colors, alpha=0.7)
            axes[0, 1].set_xlabel('Disease Risk Index')
            axes[0, 1].set_title(f'Disease Risk Assessment - {location_name}')
            axes[0, 1].set_xlim(0, 1)
            axes[0, 1].grid(True, alpha=0.3)
            
            # Add value labels
            for i, (bar, risk) in enumerate(zip(axes[0, 1].patches, df['Disease Risk'])):
                axes[0, 1].text(bar.get_width() + 0.01, bar.get_y() + bar.get_height()/2,
                              f'{risk:.2f}', va='center', fontsize=8)
            
            # Plot 3: Scatter plot
            colors_scatter = []
            for suit, risk in zip(df['Suitability Score'], df['Disease Risk']):
            ...
            scatter = axes[1, 0].scatter(df['Suitability Score'], df['Disease Risk'], 
                                        c=colors_scatter, s=100, alpha=0.7, edgecolor='black')
            axes[1, 0].set_xlabel('Suitability Score')
            axes[1, 0].set_ylabel('Disease Risk Index')
            axes[1, 0].set_title(f'Suitability vs Disease Risk - {location_name}')
            axes[1, 0].grid(True, alpha=0.3)
            
            # Add crop labels
            for i, txt in enumerate(df['Crop']):
                axes[1, 0].annotate(txt, (df['Suitability Score'].iloc[i], df['Disease Risk'].iloc[i]),
                                   xytext=(5, 5), textcoords='offset points', fontsize=8)
            
            # Plot 4: Component scores
            crops_to_show = df.nlargest(5, 'Suitability Score')['Crop']
            subset = df[df['Crop'].isin(crops_to_show)]
            
            x = np.arange(len(crops_to_show))
            width = 0.2
            
            axes[1, 1].bar(x - width*1.5, subset['Moisture Score'], width, label='Moisture', alpha=0.7)
            axes[1, 1].bar(x - width*0.5, subset['OM Score'], width, label='OM', alpha=0.7)
            axes[1, 1].bar(x + width*0.5, subset['Texture Score'], width, label='Texture', alpha=0.7)
            axes[1, 1].bar(x + width*1.5, subset['Temp Score'], width, label='Temperature', alpha=0.7)
            
            axes[1, 1].set_xlabel('Crop')
            axes[1, 1].set_ylabel('Component Score')
            axes[1, 1].set_title('Component Scores for Top 5 Crops')
            axes[1, 1].set_xticks(x)
            axes[1, 1].set_xticklabels(crops_to_show, rotation=45, ha='right')
            axes[1, 1].legend()
            ...
            st.error(f"Error creating matplotlib visualization: {e}")
            return None

# Main content area - Tabs interface
...
with tab1:
    ...
    with col1:
        ...
        country = st.selectbox("Select Country", countries, key="country_tab1")
    ...
    with col2:
        ...
        region = st.selectbox("Select Region/State", region_options, key="region_tab1")
    ...
    with col3:
        ...
        municipality = st.selectbox("Select Municipality", municipality_options, key="municipality_tab1")
    
    with col4:
        st.markdown("### Custom Soil Parameters")
        use_custom = st.checkbox("Use Custom Soil Parameters", value=True, key="use_custom_tab1")
        
        if use_custom:
            col_a, col_b = st.columns(2)
            with col_a:
                moisture_val = st.slider("Soil Moisture (m³/m³)", 0.0, 0.5, 0.15, 0.01, key="moisture_tab1")
                som_val = st.slider("Soil Organic Matter (%)", 0.0, 10.0, 2.0, 0.1, key="som_tab1")
            with col_b:
                texture_options = list(SOIL_TEXTURE_CLASSES.values())
                texture_val = st.selectbox("Soil Texture", texture_options, index=texture_options.index("Loam"), key="texture_tab1")
                temp_val = st.slider("Temperature (°C)", 0.0, 40.0, 22.0, 0.5, key="temp_tab1")
        else:
            moisture_val = 0.15
            som_val = 2.0
            texture_val = "Loam"
    ...
    # Run Analysis Button
    st.markdown("---")
    run_analysis = st.button("🚀 Run Comprehensive Analysis", type="primary", use_container_width=True, key="run_analysis_tab1")
...
with tab2:
    if 'analysis_started' in st.session_state and st.session_state['analysis_started']:
        ...
        if st.session_state['analysis_type'] == "Crop Suitability":
            ...
            col1, col2 = st.columns(2)
            with col1:
                sort_by = st.selectbox("Sort by", ["Suitability Score", "Disease Risk", "Crop Name"], key="sort_tab2")
            with col2:
                sort_order = st.selectbox("Order", ["Descending", "Ascending"], key="order_tab2")
...
with tab3:
    if 'analysis_started' in st.session_state and st.session_state['analysis_started']:
        ...
        viz_type = st.selectbox(
            ...
            ["Crop Suitability Chart", "Groundwater Analysis", "Soil Properties", "Climate Data", "Matplotlib Charts"]
        )
        ...
        if viz_type == "Crop Suitability Chart":
        ...
        elif viz_type == "Matplotlib Charts":
            ...
            st.markdown("#### Matplotlib Visualization (Alternative to Seaborn)")
            
            fig = analyzer.create_matplotlib_visualization(results_df, st.session_state['selected_location'])
            if fig:
                st.pyplot(fig)
            else:
                st.warning("Could not create matplotlib visualization")
