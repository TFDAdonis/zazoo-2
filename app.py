...
class SimplifiedClimateSoilAnalyzer:
    ...
    def create_comprehensive_climate_charts(self, climate_df, location_name):
        ...
        if 'volumetric_soil_water_layer_1' in climate_df.columns:
            months = climate_df['month_name'].unique()
            x = range(len(months))
            ...
            for i, (layer, label, color) in enumerate(zip(layer_names, layer_labels, colors)):
                if layer in climate_df.columns:
                    monthly_avg = climate_df.groupby('month')[layer].mean().reindex(range(1, 13), fill_value=0)
                    ax1.plot(x, monthly_avg.values, marker='o', color=color, linewidth=2, label=label)
        ...
        if 'total_precipitation' in climate_df.columns and 'potential_evaporation' in climate_df.columns:
            ...
            precip_monthly = climate_df.groupby('month')['total_precipitation'].sum().reindex(range(1, 13), fill_value=0)
            evap_monthly = climate_df.groupby('month')['potential_evaporation'].mean().reindex(range(1, 13), fill_value=0)
            
            width = 0.35
            ax2.bar([i - width/2 for i in x], precip_monthly.values, width, 
                    label='Precipitation', color='#36A2EB', alpha=0.8)
            ax2.bar([i + width/2 for i in x], evap_monthly.values, width, 
        ...
        if 'total_precipitation' in climate_df.columns and 'potential_evaporation' in climate_df.columns:
            ...
            ax3.plot(months, precip_values, 'b-', linewidth=2, label='Precipitation', marker='o')
            ax3.plot(months, evap_values, 'r-', linewidth=2, label='Evaporation', marker='s')
            
            # Fill between for water surplus/deficit
            ax3.fill_between(months, precip_values, evap_values, 
